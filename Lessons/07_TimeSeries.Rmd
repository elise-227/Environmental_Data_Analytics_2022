...
GaringerOzone.monthly <- GaringerOzone_clean %>%
  ...
  mutate(Date = as.Date(paste(Month,"-", 1, "-", Year), "%m - %d - %Y")) %>%
  select(Date, mean.monthly.ozone)
```
...
#only one variable decomposed at a time 
...
#add new col, zoo package, na.approx - interpolate missing data
#name of package column twice and then name of function
...
#connected the dots 
#never na omit for data in the middle of dataset 
```
