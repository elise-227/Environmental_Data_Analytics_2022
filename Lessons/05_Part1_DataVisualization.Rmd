...
O3plot2 <- ggplot(EPAair) +
  ...
  #have to add color outside of aes (not adding a layer)
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? 
#different shape for different year and colored by site name
PMplot <- 
...
#facet is site name, one plot for each site 
PMplot.faceted <-
...
#can subset in ggplot 
#facet grip, separating by two categories year and site name
PMplot.faceted2 <- 
...
# Adjust axes
#only show area want to show, not outlier points 
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
#depth better visualized with 0 at top than bottom
tempvsdepth <- 
...
NvsP <-
  #why color inside aesthetics this time?
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill" fills in instead of contour?
print(Nutrientplot3)

# Dot plot
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_dotplot(aes(color = nutrient, fill = nutrient), binaxis = "y", binwidth = 1, #stacking by y
               stackdir = "center", position = "dodge", dotsize = 2) #position doge don't want to overlay
print(Nutrientplot4)

# Violin plot, summary stats and density 
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
#use gathered data set, now in one column 
Nutrientplot7 <-   
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) #fill because color inside curve 
print(Nutrientplot6)
