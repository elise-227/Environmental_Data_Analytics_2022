...
#paste into notepad to get the number of lines to skip 
...
energy <- data.frame("States" = the_states,
                     "Price" = as.numeric(the_price),
                     "Capacity" = as.numeric(gsub(",","",the_capacity)),
                     "Net Gen" = as.numeric(gsub(",","",net_generation)),
                     "Tot Retail" = as.numeric(gsub(",","",total_retail)),
                     "Year" = 2019)
```
...
the_dfs <- lapply(x = the_years, FUN = scrape.it) %>% bind_rows()
the_dfs2 <- map(the_years,scrape.it) %>% bind_rows()
