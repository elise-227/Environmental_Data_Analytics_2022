...
NTL.phys.data.1990_99.Tues <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999))
```
Question: Why don't we filter using row numbers?

> Answer: because that is dumb and tedious 
...
```{r}
#filter is used for rows and select is for columns and rearranging columns
...
NTL.phys.data.Tues.90.99.July2 <-
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>%
  filter(year4 %in% c(1990:1999)) %>%
...
NTL.phys.data.Tues.90.99.July.surface <-
  NTL.phys.data.Tues.90.99.July %>%
  filter(depth == 0)

NTL.phys.data.Tues.90.99.July.surface.2 <-
  NTL.phys.data.Tues.90.99.July2 %>%
  filter(depth == 0)

mean(NTL.phys.data.Tues.90.99.July.surface$temperature_C)
mean(NTL.phys.data.Tues.90.99.July.surface.2$temperature_C)
```
...
irradiance <- pivot_longer(NTL.phys.data,irradianceWater:irradianceDeck, names_to = "Location", values_to = "Irradiance")


#Exercise 6: spread temperatureC into more than one column based on the depth
tempbydepth <- pivot_wider(NTL.phys.data, names_from = depth, values_from = temperature_C)
```
```
