...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) + #increase font size 
  theme(axis.text = element_text(color = "black"), #change color of text to black
        legend.position = "top")# change position of legend #alternative: legend.position + legend.justification
...
O3plot5 <- ggplot(EPAair) +
  ...
  mytheme
print(O3plot5)  
#black text and bigger font 

theme_set(mytheme) #theme for all subsequent plots 
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #horizontal lines, dashed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + #alpha trans
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), 
    date_breaks = "2 months", date_labels = "%b %y") + #month year for date
  ylab(expression("O"[3]* " AQI Value")) +
  theme(axis.text.x = element_text(angle = 45,  hjust = 1)) #angle text
print(O3plot7)  
...
#cells of diff sizes with rel_heights
```
