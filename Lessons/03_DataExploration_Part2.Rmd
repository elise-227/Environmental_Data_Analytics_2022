...
# Check our date column, will tend to input as character instead of date
...
#bin width units - default is 30 units 
ggplot(USGS.flow.data) +
...
#scale sets zooms into where data is 
...
#plot one value but color by mean approval. Color categorical value
ggplot(USGS.flow.data) +
...
#slice data with group, dist among slices of data, like histograms
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plot, range and distribution, IQR and width distribution of data
ggplot(USGS.flow.data) +
